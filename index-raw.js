...
module.exports = class DeploymentTools {
  constructor (credentials, event, callback, bucketName, gitHookKey, gitAPIkey, path, targetBranch) {
    ...
    this.getFilesFromGit(this.targetBranch);
    ...
    return new Promise((resolve, reject) => {
      const requestCallback = (error, response, body) => {
        ...
        bodyObj.forEach((fileObject, index) => {
          this.files[branchName].push(fileObject);
        });

        console.log('Files', this.files[branchName]);

        return resolve(this.files[branchName].length);
      };
  ...
  getListDiffFiles() {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Find the new files
      let notPresentInData = this.files[this.targetBranch].filter(val => !this.files[this.PRBranchName].includes(val));
      this.files.toTest.concat(notPresentInData);

      const oldFile = new Promise((resolve, reject) => {});
      const newFile = new Promise((resolve, reject) => {});

      Promise.all([oldFile, newFile]).then(function(values) {
        console.log(values);
      });
